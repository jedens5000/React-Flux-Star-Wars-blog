...
export const Cards = ( props, data, store ) => {
    ...
    <Card className="bg-dark" style={{ width: "30rem" }}>
        ...
        <Card.Img
            ...
            currentTarget.src="https://upload.wikimedia.org/wikipedia/en/6/6d/Tatooine_(fictional_desert_planet).jpg";}} 
        />        
        <Card.Body>
            ...
            <Card.Text>
                {<div>
                    ...
                    {/* <p>{props.url}</p> */}
                    <p>{props.population ? (`Population: ${props.population.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`) : ''}</p>
                    <p>{props.climate ? (`Climate: ${props.climate}`) : ''}</p>
                    <p>{props.terrain ? (`Terrain: ${props.terrain}`) : ''}</p>
                    <p>{props.surface_water ? (`Surface Water: ${props.surface_water}%`) : ''}</p>
                    <p>{props.classification ? (`Classification: ${props.classification}`) : ''}</p>
                    <p>{props.language ? (`Language: ${props.language}`) : ''}</p>
                    <p>{props.vehicle_class ? (`Class: ${props.vehicle_class}`) : ''}</p>
                    <p>{props.passengers ? (`Passengers: ${props.passengers}`) : ''}</p>
                </div>}            
            </Card.Text>
            <Button className="bg-light text-primary" 
                    href={imgPath} 
